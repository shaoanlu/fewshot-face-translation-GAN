...
from .custom_layer import TorchBatchNorm2D

class FAN():
    def __init__(self, path_to_weights_file="./detector/FAN/2DFAN-4.h5"):
        self.net = load_model(path_to_weights_file, custom_objects={'TorchBatchNorm2D': TorchBatchNorm2D})

    def detect_landmarks(self, image, bounding_box, face_detector=None):
        detect = False if bounding_box else True
        prep_img, center, scale = preprocessing_FAN(image, detect=detect)
        pred = self.net.predict(prep_img[np.newaxis, ...])
        pnts, pnts_orig = self.get_preds_fromhm(pred[-1], center, scale)
        return pnts, pnts_orig
    
    def preprocessing_FAN(self, img, detect=False, face_detector=None):
        """
        ...
        if detect:
            ...
            bbox = face_detector.detect_face(image)[0]
